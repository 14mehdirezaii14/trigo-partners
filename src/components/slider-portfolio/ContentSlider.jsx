        ...
        <div className=" hero-content">
            {data.map((item, key) =>
                <div className={`slide-content ${activeClass === key ? 'dsn-active' : ''}`} key={key}
                    ...
                    {item.description && <div className="dsn-description mt-30 text-dark">{item.description}</div>}
                    <ButtonDefault to={getPortfolioLink(item)} className="mt-30" text="About Us"
                                   icon={faAngleRight}/>
                </div>
            )}
